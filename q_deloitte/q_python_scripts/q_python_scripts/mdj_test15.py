...
def main(BATCH_ID, FILENAME):
    ...
    for tf in FILENAME:
        ...
        ''' Commented out this section of code showing raw transform passed to execute
        ...
        ##regex = re.compile('raise')
        ###query_string = re.sub(r'(?i)RAISE', 'XXXXX_RAISE_XXXXX', query_string)
        # OR 
        query_string = re.sub(r'(DO *$$)', 'XXXXX_RAISE_XXXXX', query_string, flags=re.I)

        print('new_query_string-->%s' %(query_string))
        #### mdj end
        ...
        ''' Amended this to if True, as we wish to print it in all cases, now that we have commented out the section above
