...
def decrementBuild(sqlfilebuildnumber,sqlfilecontent,is_force_mode=False,logstring='No logstring'):
    if is_force_mode:
        try: 
            cursor.execute(sqlfilecontent)
        except psycopg2.Error, e:
...
def substitute_parameters(sql_statement):
    for parametername in dir(redshiftconfig):
        if not parametername.startswith("__"):
    ...
    sql_statement = sql_statement.replace('{aws_access_key_id}', aws_access_key_id)
    sql_statement = sql_statement.replace('{aws_secret_access_key}', aws_secret_access_key)
