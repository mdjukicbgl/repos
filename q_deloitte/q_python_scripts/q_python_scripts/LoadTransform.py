...
def main(BATCH_ID, FILENAME):
    ...
    for tf in FILENAME:
        ...
        #
        # feature/4352 - mdj added 20170726 to strip out comment lines within Transform Script prior
        #                to execution of Transform script
        #      
        print('before - list sqlstring prior to loop-->%s' %(sqlstring))


        regex = re.compile(r'[^-]*--.*(\n)*.*$')
        sqlstring = [line for line in sqlstring if not regex.search(line)]
 
        print('after - list sqlstring prior to loop-->%s' %(sqlstring))
 

        for line in sqlstring:
           print('\n\nline-->%s' %(line))
